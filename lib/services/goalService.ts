...
export async function listGoals(includeArchived = false) {
  ...
  const goals = await SavingsGoal.find(filter).populate("linkedAccountId", "name balance").sort({ createdAt: -1 }).lean();

  return goals.map((g) => ({
    ...g,
    currentAmount: g.linkedAccountId ? (g.linkedAccountId as unknown as { balance: number }).balance : g.currentAmount,
  }));
}
